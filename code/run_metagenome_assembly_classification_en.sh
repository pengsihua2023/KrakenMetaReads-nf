...
echo "   - Mapping tools (Bowtie2): Apptainer containers"
echo "   - Abundance calculation (Python, Samtools, Biopython): Conda + auto symlink"
...
echo "   libbz2 dependency resolved via automatic symbolic linking"
echo ""
...
echo "   3. Reads mapping and abundance calculation (RPM/RPKM)"
echo "   4. Kraken2 taxonomic classification"
...
if [ $? -eq 0 ]; then
    ...
    if [ -d "results" ]; then
        ...
        # Check abundance results
        if [ -d "results/abundance_megahit" ]; then
            echo "  ✅ MEGAHIT abundance (RPM/RPKM): results/abundance_megahit/"
            ABUNDANCE_MEGAHIT=$(find results/abundance_megahit -name "*_abundance.txt" | wc -l)
            echo "     - Generated $ABUNDANCE_MEGAHIT abundance files"
        fi
        
        if [ -d "results/abundance_spades" ]; then
            echo "  ✅ SPAdes abundance (RPM/RPKM): results/abundance_spades/"
            ABUNDANCE_SPADES=$(find results/abundance_spades -name "*_abundance.txt" | wc -l)
            echo "     - Generated $ABUNDANCE_SPADES abundance files"
        fi
        ...
        echo ""
        echo "  Abundance results (RPM/RPKM):"
        find results/abundance_* -name "*_abundance.txt" 2>/dev/null | head -10
        echo ""

...
- **Viral Identification**: viralFlye (post-processing of metaFlye results) 🦠
  - Identifies **DNA viruses** (phages, large DNA viruses)
  - Linear viral contigs (partial or linear viral genomes)
...
- **Abundance**: RPM/RPKM for three contig sets:
  - Set 1: metaFlye all contigs (bacteria + viruses + eukaryotes) ⭐
  - Set 2: viralFlye linear viral contigs (DNA viruses) ⭐
  - Set 3: viralFlye circular viral contigs (complete DNA viral genomes) ⭐
- **Classification**: Kraken2 for all three contig sets

**Note**: This workflow detects **DNA viruses only**. For RNA virus detection (e.g., influenza, coronavirus), use Nanopore RNA-seq or Direct RNA-seq with RNA-specific assembly tools.
...
- `--run_viralflye`: Enable/disable viralFlye (default: `true`)
- `--viralflye_hmm`: **Required** Pfam-A.hmm file path for viral protein annotation
- `--viralflye_min_length`: Min viral contig length (default: `2000` bp)
  - Captures small DNA viruses (e.g., Circoviridae ~2kb)
  - Phages and large DNA viruses (>5kb)
  - **Note**: RNA viruses require RNA-seq, not DNA-seq
- `--viralflye_completeness`: Completeness cutoff (default: `0.5`, i.e., 50%)
  - Ensures identified viruses are relatively complete genomes
  - Circular contigs = complete viral genomes ⭐
- `--viralflye_threads`: Threads for viralFlye (default: 10)
...
- Identifies linear and circular DNA viral genomes
- Filters by length (≥2kb) and completeness (≥50%)
- Uses BLAST and Pfam HMM for viral protein annotation
- **Detects DNA viruses only** (phages, large DNA viruses, small DNA viruses)

**Note**: viralFlye must be pre-installed in `viralFlye_env` conda environment (not available in standard conda channels)

### Workflow Strategy
```
DNA long reads → metaFlye assembly → viralFlye identification → DNA Viral contigs
                      ↓                        ↓
...
1. **Set 1 - metaFlye all contigs**: Complete metagenome (bacteria + DNA viruses + eukaryotes)
2. **Set 2 - viralFlye linear**: Linear DNA viral genomes (partial or linear viruses)
3. **Set 3 - viralFlye circular**: Circular DNA viral genomes (complete phages, small DNA viruses) ⭐
...
### Key Parameters
- `viralflye_min_length = 2000` - Min contig length (2kb for small DNA viruses)
- `viralflye_completeness = 0.5` - Min completeness (50% for quality)
- `viralflye_threads = 10` - CPU threads
- `viralflye_hmm` - **Required** Pfam-A.hmm path for protein annotation
...
⚠️ viralFlye detects **DNA viruses only** (not RNA viruses like influenza)  
...
#### ✅ DNA Viruses (Detectable)
- **Bacteriophages** (e.g., T4, λ phage, Ralstonia phage)
- **Large DNA viruses** (e.g., Herpesviruses, Poxviruses, Megaviruses)
- **Small DNA viruses** (e.g., Circoviridae ~2kb)
- **Adenoviruses**
- **Papillomaviruses**

**Why**: DNA sequencing captures DNA viruses directly from samples.

#### ❌ RNA Viruses (NOT Detectable)
- **Influenza viruses** (8 segmented RNA genome)
- **Coronaviruses** (SARS-CoV-2, MERS, etc.)
- **HIV** (retrovirus, unless integrated into host genome)
- **Noroviruses**
- **Dengue, Zika** viruses

**Why**: DNA sequencing does NOT capture RNA virus genomes (they are RNA, not DNA).

### For RNA Virus Detection

**Required approach**: Nanopore RNA-seq or Direct RNA-seq

```bash
# Sample → RNA extraction (not DNA!)
# → cDNA synthesis OR Direct RNA-seq
# → Nanopore sequencing
# → RNA assembly tools (Trinity, rnaSPAdes, or Flye with cDNA)
# → Kraken2 classification
```

**Key differences**:
- **Input**: RNA extracted samples
- **Library prep**: cDNA or Direct RNA-seq kit
- **Assembly**: RNA-specific tools
- **Result**: Can detect both DNA and RNA viruses (if total nucleic acid extraction)

### Kraken2 Capability

**Important**: Kraken2 can classify BOTH DNA and RNA viruses!

However, it can only classify what's in your sequencing data:
- DNA-seq data → Only DNA viruses can be found
- RNA-seq data → Both RNA and DNA viruses (if present)

The limitation is in the **sample preparation and sequencing**, not Kraken2.

### Summary Table

| Virus Type | DNA-seq (Current) | RNA-seq | Example |
|------------|-------------------|---------|---------|
| **DNA viruses** | ✅ Detected | ❌ Not captured | Phages, Megavirus |
| **RNA viruses** | ❌ Not captured | ✅ Detected | Influenza, COVID-19 |
| **Retroviruses** | ⚠️ If integrated | ✅ Detected | HIV |
